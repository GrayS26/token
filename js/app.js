(() => {
    ...
    function initSliders() {
        if (document.querySelector(".roadmap__slider")) {
            const mainSlider = new swiper_core_Swiper(".roadmap__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 2,
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        const fullLine = document.querySelector(".line");
        const rocket = document.querySelector(".rocket");
        function animateRocket() {
            const svgRect = svgContainer.getBoundingClientRect();
            const lineLength = fullLine.getTotalLength();
            rocket.getBoundingClientRect();
            const rocketStyle = rocket.style;
            let startTime;
            function frame(timestamp) {
                if (!startTime) startTime = timestamp;
                const duration = 3500;
                const progress = (timestamp - startTime) / duration;
                const offset = progress * lineLength;
                if (progress <= 1) {
                    const point = fullLine.getPointAtLength(offset);
                    const xPercent = point.x / svgRect.width * 100;
                    const yPercent = point.y / svgRect.height * 100;
                    rocketStyle.left = `${xPercent}%`;
                    rocketStyle.top = `${yPercent}%`;
        ...
        function handleScroll() {
            ...
            const isInView = svgContainerRect.top < window.innerHeight && svgContainerRect.bottom >= 0;
            if (isInView && !svgContainer.classList.contains("_watcher-view")) {
                svgContainer.classList.add("_watcher-view");
                animateRocket();
            }
        }
